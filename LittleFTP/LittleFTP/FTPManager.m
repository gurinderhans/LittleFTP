...
- (id)init
{
	...
	if (self) {
		...
		self.onProgress = ^(double totalProgress, NSString* fileName){};
	}
...
- (BOOL) _uploadFile:(NSURL*)fileURL toServer:(FMServer*)server atLocation:(NSString *)serverPath {
	BOOL success = YES;
	...
	NSURL * finalURL = [[server.destination ftpURLForPort:server.port] URLByAppendingPathComponent: serverPath];
	And(success, (finalURL != nil));
	Check(success);
...
- (NSArray*) _contentsOfServer:(FMServer*)server atLocation:(NSString *)subpath {
	BOOL success = YES;
	...
	// appending empty string ensures the path is a directory
	dest = [[dest URLByAppendingPathComponent:subpath] URLByAppendingPathComponent:@""];
...
- (BOOL) uploadFile:(NSURL*)fileURL toServer:(FMServer*)server atLocation:(NSString *)serverPath {
	if (![self _checkFMServer:server]) {
		return NO;
	}
	if (!fileURL) {
	...
	return RunInSeparateThread([self _uploadFile:fileURL toServer:server atLocation:serverPath]);
}
...
- (NSArray*) contentsOfServer:(FMServer*)server atLocation:(NSString *)subpath {
	if (![self _checkFMServer:server]) {
		return nil;
	}
	return RunInSeparateThread([self _contentsOfServer:server atLocation:subpath]);
}
...
//						}
						...
						self.onProgress(((float)fileSizeProcessed/(float)fileSize)*100, mFileName);
					}
				}
			} else {
				//something went wrong here...
...
+(FMServer*)serverWithDestination:(NSString*)dest onPort:(int)port username:(NSString*)user password:(NSString*)pass {
